...
public:
  ...
  CalibrationMatrix getDefaultCalibrationData(int disp);
  void drawCalibPoint(uint16_t x, uint16_t y);
  void drawCalibration(CalibrationMatrix calibrationMatrix);
  void printCalibration(CalibrationMatrix calibrationMatrix);
private:
  DmTftBase* _tft;
  DmTouch* _touch;
  uint16_t _textRow;
  ...
  void releaseTouch(uint16_t x, uint16_t y);
